...
class BatchTripletLoader(Loader):
  ...
  def load_imgs(self, paths, labels, img_shape, img_per_class=None, shuffle=True):
    self.__loadimgs__(paths, labels, img_shape, img_per_class, shuffle)
    self.mode = 1
    
  def load_paths(self, paths, labels, img_shape):
    # validate the arguments -> all_paths/all_classes
    args = [paths, labels]
    for i, arg in enumerate(args):
      if type(arg) == list:
  ...
  def get_batch_semi_hard(self, img_per_class, model = None, hard_margin = 0.8, shuffle = True):    
  ...
  def get_batch_hard(self, img_per_class, model = None, hard_margin = 0.8, shuffle = True):      
    # get the randomly formed triplet pairs
    self.get_batch_random(img_per_class=img_per_class, shuffle = False)
